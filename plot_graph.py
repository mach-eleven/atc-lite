...
def save_graph():
    ...
    env_name = 'AtcGym-v0'
    model_name = "ppo"
    log_dir = Path("logs") / model_name / env_name
    ...
    current_num_files = list(log_dir.glob('*/log.txt'))
    ...
    for run_num in range(1, num_runs+1):
        # log file name
        log_f_name = log_dir / f'run_{run_num}' / 'log.txt'
        if not os.path.exists(log_f_name):
            print(f"Log file not found for run {run_num}")
            continue
        # log file name
