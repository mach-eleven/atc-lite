...
from utils.type_checkers import curr_stage_type, entry_point_type, gt_0, checkpoint_type, gt_0_float, outdir_type, log_list_type, parse_log_config
import logging
...
def validate_and_get_entry_point(entry, heading, level, curr_stage_entry_point):
    if entry is not None and heading is not None and level is not None:
        ...
        if curr_stage_entry_point < 1 or curr_stage_entry_point > args.curr_stages:
            raise ValueError(f"Curriculum stage {curr_stage_entry_point} is out of range. Must be between 1 and {args.curr_stages}.")
        entry_point = scenarios.SupaSupa().generate_curriculum_entrypoints(num_entrypoints=args.curr_stages)[curr_stage_entry_point - 1]
...
def add_arguments(parser):
    ...
    parser.add_argument('--skip-frames', type=gt_0, default=100, help='Render every Nth frame (default: 1, i.e., no skipping)')
...
if __name__ == "__main__":
    ...
    entry_point = validate_and_get_entry_point(args.entry, args.heading, args.level, args.curr_stage_entry_point)
    ...
    model_ = PPO.load(args.checkpoint)
    obs = env.reset()[0]
    done = False
    frame_count = 0
    while not done:
