...
def train_generalized_model(args, reward_keys, scenario=None, num_airplanes=1):
    ...
    for stage, entry_point_choices in enumerate(curriculum_entry_points, start=0):
        ...
        elif stage > 0 and (prev_model_path := stage_dir.parent / f"curr_gen_model_{stage_name}.zip").exists():
            model_ = PPO.load(
                prev_model_path,
                env=vec_env,
                verbose=1 if args.debug else 0,
        ...
        try:
            ...
            for ep in range(args.max_episodes):
                ...
                if ep % 100 == 0:
                    ...
                    scenario_code = 1
                # Every 10 episodes, we will change the entry point
                entry_point = entry_point_choices[ep % len(entry_point_choices)]
                # Log the chosen scenario and entry point
                logger.info(f"Chosen scenario: {chosen_scenario}, Entry point: {entry_point}")
                
                # Create environment with the current curriculum entry point
                env = my_env(scenario_code, chosen_scenario, entry_point)
                vec_env = make_vec_env(my_env, n_envs=args.threads, env_kwargs={"scenario": scenario_code, "scenario_obj": chosen_scenario, "curriculum_entry_point": entry_point}, vec_env_cls=SubprocVecEnv)
                vec_env.reset()
                env.reset()
